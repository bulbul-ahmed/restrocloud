...
const STATUS_STYLES: Record<TableStatus, { border: string; bg: string; text: string; dot: string }> = {
  AVAILABLE:      { border: 'border-emerald-300', bg: 'bg-emerald-50 hover:bg-emerald-100',  text: 'text-emerald-800', dot: 'bg-emerald-500' },
  OCCUPIED:       { border: 'border-brand/40',   bg: 'bg-brand/10 hover:bg-brand/20',     text: 'text-brand',   dot: 'bg-brand' },
  BILL_REQUESTED: { border: 'border-amber-300',   bg: 'bg-amber-50 hover:bg-amber-100',       text: 'text-amber-800',   dot: 'bg-amber-500' },
  RESERVED:       { border: 'border-blue-300',    bg: 'bg-blue-50 hover:bg-blue-100',         text: 'text-blue-800',    dot: 'bg-blue-500' },
  CLEANING:       { border: 'border-violet-300',  bg: 'bg-violet-50 hover:bg-violet-100',     text: 'text-violet-800',  dot: 'bg-violet-500' },
  OUT_OF_SERVICE: { border: 'border-gray-200',    bg: 'bg-gray-50',                           text: 'text-gray-400',    dot: 'bg-gray-300' },
  ...
  return (
    <button
      ...
      className={cn(
        ...
        table.status === 'OUT_OF_SERVICE' && 'cursor-not-allowed opacity-50',
      )}
    >
      {/* Status dot */}
      <span
      ...
      <p className={cn('font-bold text-lg leading-tight', style.text)}>
        {table.tableNumber}
      </p>
      ...
      {activeSession && (
        <div className="mt-2 pt-2 border-t border-black/5">
          <p className="text-xs text-gray-600">
            <span className="font-medium">{activeSession.guestCount}</span> guests
          </p>
          {orderTotal > 0 && (
          ...
          {activeSession.status === 'BILL_REQUESTED' && (
            <Badge variant="warning" className="text-2xs mt-1">Bill Requested</Badge>
          )}
        </div>
      )}
    </button>
...
function FloorPanel({
  ...
  return (
    <div className="w-[340px] flex-shrink-0 flex flex-col border-r border-border bg-white overflow-hidden">
      {/* Stats bar */}
      <div className="px-4 py-3 border-b border-border bg-surface-subtle flex-shrink-0">
        ...
        <div className="flex gap-1 px-3 py-2 border-b border-border overflow-x-auto flex-shrink-0">
          <button
            ...
            className={cn(
              ...
              activeSection === null
                ? 'bg-brand text-white'
                : 'bg-surface-subtle text-gray-600 hover:bg-gray-100',
            )}
          >
            All
          </button>
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-1.5">
        ...
        {filtered.map((item) => (
          <div
            key={item.id}
            className="flex items-center gap-3 p-2.5 rounded-lg border border-border bg-white hover:bg-surface-subtle transition-colors"
          >
            ...
            <button
              ...
              className="flex items-center justify-center w-7 h-7 rounded-full bg-brand text-white hover:bg-brand/90 transition-colors flex-shrink-0"
            >
              <Plus size={14} />
            </button>
          </div>
...
export default function POSPage() {
  ...
  const placeOrderMutation = useMutation({
    mutationFn: () => {
      ...
      return posApi.createOrder(restaurantId, {
        channel: 'DINE_IN',
  ...
  const quickOrderMutation = useMutation({
    mutationFn: (params: {
      ...
      posApi.createOrder(restaurantId, {
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 64px)' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <OrderPanel
          ...
          onSeatGuests={() => setSeatDialogOpen(true)}
          onRequestBill={() => requestBillMutation.mutate()}
          onCloseSession={(force) => closeSessionMutation.mutate(force)}
          onMarkAvailable={() => markAvailableMutation.mutate()}
          onPay={setPayDialog}
          ...
          onTransferTable={(targetTableId) => transferSessionMutation.mutate(targetTableId)}
